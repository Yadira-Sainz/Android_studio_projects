    ...
    private class SwarmInitializerAgentBehaviour extends OneShotBehaviour {
        ...
        public void action() {
            ...
            for (int i = 0; i < swarmSize; i++) {
                ...
                swarm[i].position = new double[3]; // Asumiendo que hay tres dimensiones como por la matriz OUTPUT
                swarm[i].velocity = new double[3];
                for (int j = 0; j < swarm[i].position.length; j++) {
                    swarm[i].position[j] = Math.round(randomInRange(0, 10)); // LB es 0 y UB es 10
            ...
            // Imprimir resultados
            System.out.println("Mejor posición global: B0 = " + gBestPosition[0] + ", B1 = " + gBestPosition[1] + ", B2 = " + gBestPosition[2]);
